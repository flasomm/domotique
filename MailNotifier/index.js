...
MailNotifier.prototype.init = function (config) {
        ...
        handler: function (command, args) {
            ...
            if (command !== 'update') {
                ...
                if (!mailObject.mail_to || mailObject.mail_to === '') {
                    self.addNotification('error', 'Missing receiver e-mail address. Please check your configuration in the following app instance: ' +
    ...
    this.notificationMailWrapper = function (notification) {
        console.log('######===============>>>>>>>>>>>>>> notification1', JSON.stringify(notification));
...
MailNotifier.prototype.sendSendMessageWithDelay = function () {
    ...
    this.timer = setInterval(function () {
        ...
        if (self.collectMessages.length > 0) {
            ...
            const message = self.vDev.get('metrics:message') !== '' ? self.vDev.get('metrics:message') : mailObject.message;

            var sendEmail = system('/usr/bin/sendemail -f me@z-wave.me -t "' + mailObject.mail_to + '" -u "' + self.subject +
                '" -m "' + message + '"');

            console.log('######===============>>>>>>>>>>>>>> sendEmail', sendEmail);

            if (!sendEmail[1].includes('Email was sent successfully')) {
                self.addNotification('error', 'MailNotifier-ERROR: ' + sendEmail, 'module');
            }
