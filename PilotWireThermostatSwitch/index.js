/*** PilotWireThermostatSwitch Z-Way HA module *******************************************
 ...
 Description:
 This module creates a central heat control that can control one or multiple relay switch, like a radiator pilot wire for example.
 It's possible to create multiple virtual heat control, one for each room and schedule release for a week.
 ******************************************************************************/
...
function PilotWireThermostatSwitch(id, controller) {
    PilotWireThermostatSwitch.super_.call(this, id, controller);
}

inherits(PilotWireThermostatSwitch, AutomationModule);

_module = PilotWireThermostatSwitch;

// ----------------------------------------------------------------------------
// --- Module instance initialized
// ----------------------------------------------------------------------------

PilotWireThermostatSwitch.prototype.scheduleInputAnalyseAndAdd = function (timetable, schedule) {
...
PilotWireThermostatSwitch.prototype.init = function (config) {
    PilotWireThermostatSwitch.super_.prototype.init.call(this, config);
    var self = this;
    ...
    this.langFile = self.controller.loadModuleLang("PilotWireThermostatSwitch");
    this.schedule = self.scheduleInputAnalyseAndAdd(self.config.schedule, this.schedule);
    ...
    this.controller.on("pilotWireThermostatSwitchReset_" + this.id + ".poll", this.pollReset);
    this.controller.emit("cron.addTask", "pilotWireThermostatSwitchReset_" + this.id + ".poll", {
        minute: null,
        hour: null,
        weekDay: null,
        day: null,
...
PilotWireThermostatSwitch.prototype.stop = function () {
    var self = this;
    self.newRooms.forEach(function (room) {
        self.regTH('off', room.room);
    });

    var devID = "PilotWireThermostatSwitch_" + self.id;
    ...
    this.controller.emit("cron.removeTask", "pilotWireThermostatSwitchReset_" + this.id + ".poll");
    this.controller.off("pilotWireThermostatSwitchReset_" + this.id + ".poll", this.pollReset);

    this.controller.off('core.start', self.initialCCTurnON);

    PilotWireThermostatSwitch.super_.prototype.stop.call(this);
};
...
PilotWireThermostatSwitch.prototype.createHouseControl = function () {

    // use vdevinfo if it exists
    var self = this,
        vdevEntry = this.controller.vdevInfo["PilotWireThermostatSwitch_" + this.id] &&
        this.controller.vdevInfo["PilotWireThermostatSwitch_" +
        this.id].metrics ? this.controller.vdevInfo["PilotWireThermostatSwitch_" +
        this.id].metrics : undefined;
    ...
    this.pollByStart = function (filter) {
        ...
        if (thermostats.length > 0) {
            ...
            if (self.vDev === null) {
                self.vDev = self.controller.devices.get('PilotWireThermostatSwitch_' + self.id);
            }
    ...
    this.pollByEnd = function () {
        ...
        if (thermostats.length > 0) {
            ...
            // get all rooms controlled by pilot wire app, and update them
            // update value in widget
            metrRooms.forEach(function (room) {
                if (parseInt(room.room, 10) === locId) {
                    if (room.fallback) {
    ...
    this.configureSchedules = function (roomId) {
        var subString = 'pilotWireThermostatSwitch.' + roomId + '.',
            tempSet = false;
        ...
        _.forEach(scheduleFilter, function (scheduleEntry) {
            ...
            /*
             * filter for start /end - by index of or substring 'pilotWireThermostatSwitch.0.1.0.'
    ...
    self.vDev = self.controller.devices.create({
        deviceId: "PilotWireThermostatSwitch_" + this.id,
        defaults: {
            deviceType: "sensorMultiline",
            metrics: {
                multilineType: "pilotWireThermostatSwitch",
                icon: "pilotwirethermostatswitch",
                rooms: self.newRooms
            }
        },
        overlay: {
        ...
        handler: function (command, args) {
            ...
            self.newRooms.forEach(function (room) {
                ...
                if (argRoom === null || argRoom === roomId) {
                    ...
                    if ((argRoom === null && command === 'schedule' && room.state !== 'schedule') ||
                        (argRoom === null && command === 'custom' && roomCmd === 'schedule') ||
                        (!!argRoom && roomCmd === 'schedule')) {
                        // activate schedule
                    ...
                    if ((argRoom === null && self.vDev.get('metrics:state') === 'schedule' && command !== 'schedule') ||
                        (!!argRoom && room.state === 'schedule' && roomCmd !== 'schedule')) {
                        var subString = 'pilotWireThermostatSwitch.' + roomId + '.';
            ...
            if (!!argRoom) {
                ...
                this.set('metrics:icon', '/ZAutomation/api/v1/load/modulemedia/PilotWireThermostatSwitch/pilotwirethermostatswitch_custom.png');
            } else {
                this.set('metrics:state', command);
                this.set('metrics:icon', command ===
                'custom' ? '/ZAutomation/api/v1/load/modulemedia/PilotWireThermostatSwitch/pilotwirethermostatswitch_custom.png' : 'pilotwirethermostatswitch');
            }
    ...
    self.newRooms.forEach(function (room, i) {
        ...
        room.targetTemp = vdevEntry && vdevEntry.rooms[i] &&
        vdevEntry.rooms[i].targetTemp ? parseFloat(vdevEntry.rooms[i].targetTemp) : parseFloat(room.comfort);
...
PilotWireThermostatSwitch.prototype.performChangesOnThermostats = function (thermostat, temp) {
    var self = this;
    var entryExists = _.filter(self.fallbackOverTime, function (entry) {
...
PilotWireThermostatSwitch.prototype.initFunctions = function () {
    var self = this;
    self.resetList = {};
 ...
 */
PilotWireThermostatSwitch.prototype.regTH = function (action, roomId) {
    var self = this;
 ...
 */
PilotWireThermostatSwitch.prototype.checkEntry = function (thermostats, room) {
    var self = this;
    ...
    if (_.isArray(thermostats) && thermostats.length > 0 &&
        ...
        roomSchedules.forEach(function (rSc, index) {
            ...
            if (_.isArray(rSc.Weekday) && rSc.Weekday.length > 0) {
                ...
                self.controller.addNotification('error', self.langFile.err_wrong_date_format, 'module', 'PilotWireThermostatSwitch');
            }
        });
        ...
        self.controller.addNotification('warning', self.langFile.err_no_thermostats + roomName, 'module', 'PilotWireThermostatSwitch');
    } else {
        self.controller.addNotification('warning', self.langFile.err_parsing_schedule_data, 'module', 'PilotWireThermostatSwitch');
    }
};
 ...
 */
PilotWireThermostatSwitch.prototype.initializeSchedules = function (day, rSc, index) {
    var self = this,
    ...
    if (tempOrModus !== '') {
        ...
        if (_.isNumber(tempOrModus) && (tempOrModus < 5 || tempOrModus > 29)) {
            self.controller.addNotification('warning', self.langFile.err_temp_out_of_range, 'module', 'PilotWireThermostatSwitch');
        } else {
            setStart = tempOrModus.toString();
            setEnd = null;
        }
    } else {
        self.controller.addNotification('warning', self.langFile.err_temp_entry, 'module', 'PilotWireThermostatSwitch');
    }
    ...
    if ((startHour >= 0 && startHour < 24 ) && (endHour >= 0 && endHour < 24 ) && // check  0 >= hours < 24
        ...
        self.controller.addNotification('warning', self.langFile.err_wrong_time_format, 'module', 'PilotWireThermostatSwitch');
    } else {

        self.controller.addNotification('warning', self.langFile.err_something_went_wrong, 'module', 'PilotWireThermostatSwitch');
    }
};
 ...
 *
 */
PilotWireThermostatSwitch.prototype.createSchedule = function (startStop, min, hour, setTempOrModus, weekDay, roomId, scheduleIndex) {
    var temperature = !!setTempOrModus ? "." + setTempOrModus.replace(/\.|\,/ig, '-') : '',
        pollIdentifier = "pilotWireThermostatSwitch." + roomId + "." + scheduleIndex + "." + weekDay + "." + startStop + "." + hour + "." + min +
            temperature + ".poll";
 ...
 */
PilotWireThermostatSwitch.prototype.createFallbackThermostatSettings = function (roomId, thermostatArr) {
    var self = this;
...
PilotWireThermostatSwitch.prototype.getInstanceTitle = function (instanceId) {
    var instanceTitle = this.controller.instances.filter(function (instance) {
        return instance.id === instanceId;
    });

    return instanceTitle[0] && instanceTitle[0].title ? instanceTitle[0].title : 'Pilot Wire Thermostat Switch ' + this.id;
};
 ...
 */
PilotWireThermostatSwitch.prototype.getThermostats = function (roomId) {
    var self = this;
...
PilotWireThermostatSwitch.prototype.log = function (message, value, stringify) {
    if (stringify) {
        console.log('##################################');
        console.log('##');
