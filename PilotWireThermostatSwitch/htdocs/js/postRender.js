function modulePostRender(control) {
    ...
    var $tableRoom = $('.pilotWireThermostatSwitchRoomSettings').find('table'),
        $tableRoomHead = $tableRoom.find('thead tr'),
        $tableSheduler = $('.pilotWireThermostatSwitchSchedule').find('table'),
    ...
    $('.pilotWireThermostatSwitchRoomSettings').on('click', 'button[data-alpaca-array-toolbar-action=add]', function(event) {
        $tableRoomHead.show();
    ...
    $('.pilotWireThermostatSwitchSchedule').on('click', 'button[data-alpaca-array-toolbar-action=add]', function(event) {
       $tableShedulerHead.show();
    });
    ...
    $tableRoom.on('click', 'button', function(event) {
        ...
        switch(action) {
            ...
            case 'remove':
                ...
                if($tableRoom.find('tbody tr').length < 1) {
                    $tableRoomHead.hide();
                    $('.pilotWireThermostatSwitchRoomSettings').find('button[data-alpaca-array-toolbar-action=add]').closest('div').show();
                }
                break;
        }
    });
