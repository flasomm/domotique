...
ClimateControl.prototype.scheduleInputAnlayseAndAdd = function (timetable, schedule) {
    var self = this;
...
ClimateControl.prototype.init = function (config) {
    ...
    this.fallbackThermostatSettings = {},
        this.langFile = self.controller.loadModuleLang("ClimateControl"),
    ...
    this.controller.on("climateControlReset_" + this.id + ".poll", this.pollReset);
    this.controller.emit("cron.addTask", "climateControlReset_" + this.id + ".poll", {
        minute: null,
        hour: null,
        weekDay: null,
        day: null,
...
ClimateControl.prototype.stop = function () {
    ...
    var devID = "ClimateControl_" + self.id;
    ...
    this.controller.emit("cron.removeTask", "climateControlReset_" + this.id + ".poll");
    this.controller.off("climateControlReset_" + this.id + ".poll", this.pollReset);
...
ClimateControl.prototype.createHouseControl = function () {

    // use vdevinfo if it exists
    var self = this,
        vdevEntry = this.controller.vdevInfo["ClimateControl_" + this.id] &&
        this.controller.vdevInfo["ClimateControl_" + this.id].metrics ? this.controller.vdevInfo["ClimateControl_" +
    ...
    this.pollByStart = function (filter) {
        ...
        if (thermostats.length > 0) {
            ...
            if (self.vDev === null) {
                self.vDev = self.controller.devices.get('ClimateControl_' + self.id);
            }
    ...
    this.configureSchedules = function (roomId) {
        var subString = 'climateControl.' + roomId + '.',
            tempSet = false;
        ...
        _.forEach(scheduleFilter, function (scheduleEntry) {
            ...
            /*
             * filter for start /end - by index of or substring 'climateControl.1.0.2.'
    ...
    self.vDev = self.controller.devices.create({
        deviceId: "ClimateControl_" + this.id,
        defaults: {
        ...
        handler: function (command, args) {
            ...
            if (!!argRoom) {
                ...
                this.set('metrics:icon', '/ZAutomation/api/v1/load/modulemedia/ClimateControl/climatecontrol_custom.png');
            } else {
                this.set('metrics:state', command);
                this.set('metrics:icon', command ===
                'custom' ? '/ZAutomation/api/v1/load/modulemedia/ClimateControl/climatecontrol_custom.png' : 'climatecontrol');
            }
...
ClimateControl.prototype.performChangesOnThermostats = function (thermostat, temp) {
    var self = this;
    var entryExists = _.filter(self.fallbackOverTime, function (entry) {
...
ClimateControl.prototype.initFunctions = function () {
    var self = this;
    self.resetList = {};
 ...
 */
ClimateControl.prototype.regTH = function (action, roomId) {
    var self = this;
 ...
 */
ClimateControl.prototype.checkEntry = function (thermostats, room) {
    var self = this;
    ...
    if (_.isArray(thermostats) && thermostats.length > 0 &&
        ...
        roomSchedules.forEach(function (rSc, index) {
            ...
            if (_.isArray(rSc.Weekday) && rSc.Weekday.length > 0) {
                ...
                self.controller.addNotification('error', self.langFile.err_wrong_date_format, 'module', 'ClimateControl');
            }
        });
        ...
        self.controller.addNotification('warning', self.langFile.err_no_thermostats + roomName, 'module', 'ClimateControl');
    } else {
        self.controller.addNotification('warning', self.langFile.err_parsing_schedule_data, 'module', 'ClimateControl');
    }
};
 ...
 */
ClimateControl.prototype.initializeSchedules = function (day, rSc, index) {
    var self = this,
    ...
    if (tempOrModus !== '') {
        ...
        if (_.isNumber(tempOrModus) && (tempOrModus < 5 || tempOrModus > 29)) {
            self.controller.addNotification('warning', self.langFile.err_temp_out_of_range, 'module', 'ClimateControl');
        } else {
            setStart = tempOrModus.toString();
            setEnd = null;
        }
    } else {
        self.controller.addNotification('warning', self.langFile.err_temp_entry, 'module', 'ClimateControl');
    }
    ...
    if ((startHour >= 0 && startHour < 24 ) && (endHour >= 0 && endHour < 24 ) && // check  0 >= hours < 24
        ...
        self.controller.addNotification('warning', self.langFile.err_wrong_time_format, 'module', 'ClimateControl');
    } else {

        self.controller.addNotification('warning', self.langFile.err_something_went_wrong, 'module', 'ClimateControl');
    }
};
 ...
 * 
 */
ClimateControl.prototype.createSchedule = function (startStop, min, hour, setTempOrModus, weekDay, roomId, scheduleIndex) {
    var temperature = !!setTempOrModus ? "." + setTempOrModus.replace(/\.|\,/ig, '-') : '',
        pollIdentifier = "climateControl." + roomId + "." + scheduleIndex + "." + weekDay + "." + startStop + "." + hour + "." + min +
            temperature + ".poll";
 ...
 */
ClimateControl.prototype.createFallbackThermostatSettings = function (roomId, thermostatArr) {
    var self = this;
...
ClimateControl.prototype.getInstanceTitle = function (instanceId) {
 ...
 */
ClimateControl.prototype.getThermostats = function (roomId) {
    var self = this;
...
ClimateControl.prototype.log = function (message, value, stringify) {
    var self = this;
    if (stringify) {
        console.log('##################################');
        console.log('##');
